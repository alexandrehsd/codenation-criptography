...
Para um melhor entendimento da solução, são fornecidos dois arquivos em que se pode chegar a resposta final: o arquivo `answer.py` e um Jupyter Notebook. 
...
O arquivo `criptography-notebook.ipynb` é um espelho do arquivo `answer.py`, exceto que neste caso é mais fácil analisar linha-a-linha do código sobre o que está acontecendo enquanto o algoritmo é executado. Seu programa deve submeter o arquivo `answer.json` atualizado para correção via POST para a API do desafio.
